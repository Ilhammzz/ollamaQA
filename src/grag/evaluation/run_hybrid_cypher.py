"""Hybrd Cypher retriever evaluation workflow"""
...
from ..retrievers import create_hybrid_cypher_retriever_tool
from ...prep.encodings import REGULATION_CODES


def run_hybrid_cypher_workflow(
    ...
    *,
    ...
    ranker: Union[str, HybridSearchRanker] = HybridSearchRanker.LINEAR,
    ...
    for data in tqdm(
        ...
        desc=f"Running hybrid_cypher_retriever: `{experiment_name}`",
        disable=not verbose,
    ):
        tool_result = hybrid_cypher_retriever.invoke(
            ToolCall(
                name=hybrid_cypher_retriever.model_dump()["name"],
