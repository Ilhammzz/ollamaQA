...
# ===================== MAIN FUNCTION =====================
def generate_answer(columns, rows, question, mode: str = "zero-shot"):
    """
    Mengubah hasil query menjadi jawaban bahasa alami berdasarkan pertanyaan awal.
    Dapat memilih mode: "zero-shot" atau "few-shot"
    """
    rows_text = "\n".join([str(row) for row in rows])
    columns_text = ", ".join(columns)

    if mode == "few-shot":
        return answer_chain_few.run(columns=columns_text, rows=rows_text, question=question).strip()
    return answer_chain_zero.run(columns=columns_text, rows=rows_text, question=question).strip()
